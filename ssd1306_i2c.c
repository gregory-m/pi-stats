...
/*
* Get CPU load avarage
*/
float GetCpuUsageLA(void)
{
  double load[3];
  getloadavg(load, 3);
  return (float)load[0];
...
{
  static char hostname[HOST_NAME_LEN];
...
void LCD_DisplayInit(void)
{
  ...
  strcpy(hostname, GetHostname());    // Gets the IP address of the default interface
  OLED_ShowString(0, 0, hostname, 8); // Display IP address
}
...
void LCD_DisplayTemperature(void)
{
  ...
  sprintf_fix(cpu_perc, 3, GetCpuUsageLA()); // Gets the load on the CPU

  // OLED_Clear();              // Clear the screen

  OLED_ShowString(48, 3, temp, 8);     // Display CPU temperature
  OLED_ShowString(90, 3, cpu_perc, 8); // Display CPU load
}
